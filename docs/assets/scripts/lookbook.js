(() => {
  ...
  parcelRegister('6RQZy', function (module, exports) {
    ...
    const $5000cc5d1e9e824a$var$whiteListedAttributes = ['preview', 'scenario', 'panels', 'actions', 'param-*'];
    ...
    function $5000cc5d1e9e824a$var$createIframe(attrs) {
      ...
      const classes = $5000cc5d1e9e824a$var$attrValue(attrs, 'class', '')
        .split(' ')
        .map(c => c.trim())
        .filter(c => c.length);
    ...
    function $5000cc5d1e9e824a$var$buildSrc(attrs) {
      ...
      return encodeURI([appPath, $5000cc5d1e9e824a$var$embedUrlPrefix].join('/') + `?props=${JSON.stringify(props)}`);
    }
    ...
    function $5000cc5d1e9e824a$var$guessBasePath() {
      ...
      if (scriptSrc && scriptSrc.includes('lookbook-assets'))
        return scriptSrc.split('?')[0].replace('lookbook-assets/js/lookbook.js', 'lookbook');
  ...
  (function (undefined) {
    ...
    function iFrameListener(event) {
      ...
      function isMessageForUs() {
        return (
          msgId === ('' + msg).slice(0, msgIdLen) && msg.slice(msgIdLen).split(':')[0] in settings // ''+Protects against non-string msg
        );
      }
    ...
    function setupIFrame(iframe, options) {
      ...
      function setupBodyMarginValues() {
        if (
          'number' === typeof (settings[iframeId] && settings[iframeId].bodyMargin) ||
      ...
      function getTargetOrigin(remoteHost) {
        return '' === remoteHost || null !== remoteHost.match(/^(about:blank|javascript:|file:\/\/)/)
          ? '*'
          : remoteHost;
      }
      ...
      function processOptions(options) {
        ...
        if (settings[iframeId])
          settings[iframeId].targetOrigin =
            true === settings[iframeId].checkOrigin ? getTargetOrigin(settings[iframeId].remoteHost) : '*';
      }
      function beenHere() {
    ...
    else if (typeof $5180433265c858be$exports === 'object')
      // Node for browserfy
